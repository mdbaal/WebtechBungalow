...
def before_first_request_func():
    ...
    if not Bungalow.query.filter_by(type=type_x).first():
        db.session.add(Bungalow("Scandinavische Bungalow", type_x, "4p_scandinavische_bungalow"))
    type_x = Bungalow_Type.query.filter_by(size=6).first().id
    if not Bungalow.query.filter_by(type=type_x).first():
        db.session.add(Bungalow("Veluwse Bungalow", type_x, "6p_veluwse_bungalow"))
    type_x = Bungalow_Type.query.filter_by(size=8).first().id
    if not Bungalow.query.filter_by(type=type_x).first():
        db.session.add(Bungalow("Luxe Bungalow", type_x, "8p_luxe_bungalow"))
